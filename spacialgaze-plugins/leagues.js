...
function leagueLog(message, league) {
	let leagueid = toId(league);
	fs.writeFile(path.resolve(__dirname, 'logs/leagues/' + leagueid + '.log'), message);
}
...
function leagueTourPoints(winner, runnerup, tourSize, room) {
	...
	if (secondLeague && secondPoints > 0) {
		...
		save();
		logPoints(runnerup, secondLeague, secondPoints, "Second place in a tournament in " + room.id);
		...
		if (status === 'start') {
			...
			if (room.lvl.mode === "normal") {
				...
				if (room.lvl.status[room.lvl.statusNumber]) {
					...
					save();
					logPoints("LvL", toId(winner), points, "Won League vs League against " + loser + ".");
	...
	league: {
		...
		points: {
			give: function (target, room, user) {
				...
				save();
				//logPoints("ADMIN", league, amount, "Points given by " + user.name);
				logPoints(league, user.name + " gave " + amount + " Points");
			...
			take: function (target, room, user) {
				...
				leagues[league].points -= amount;
				save();
				//logPoints("ADMIN", league, -amount, "Points taken by " + user.name);
				logPoints(league, user.name + " took " + amount + " Points.");
			...
			reset: function (target, room, user) {
				...
				Rooms('staff').add("|raw|" + SG.nameColor(user.name, true) + " has reset all league points.").update();
