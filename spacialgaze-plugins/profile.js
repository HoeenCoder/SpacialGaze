...
function showLeague(userid) {
	if (!SG.getLeague(userid)) return '(N/A)';
	return SG.getLeague(userid) + ' (' + SG.getLeagueRank(userid) + ')';
}
	...
	profile: function (target, room, user) {
		...
		function showProfile() {
			Economy.readMoney(toId(username), currency => {
				...
				profile += '&nbsp;<font color="#24678d"><b>League:</b></font> ' + showLeague(toId(username)) + '<br />';
