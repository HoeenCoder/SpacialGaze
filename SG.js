	...
	*/
	...
	onFaint: function (userid, battle, faintData) {
		...
		while ((curExp + activeExp) >= this.calcExp(faintData.source.species, (faintData.source.level + 1))) {
			...
			battle[faintData.source.side.substr(0, 2)].pokemon[faintData.source.slot].level++;
			faintData.source.level++;
			levelUps++;
		}
		battle[faintData.source.side.substr(0, 2)].pokemon[faintData.source.slot].exp += activeExp;
		out += "|" + levelUps;
		// New evs
		...
		while (exp.length) {
			...
			while ((cur.exp + mon.exp) >= this.calcExp(mon.species, (mon.level + 1))) {
				...
				battle[faintData.source.side.substr(0, 2)].pokemon[mon.slot].level++;
				mon.level++;
			}
			battle[faintData.source.side.substr(0, 2)].pokemon[mon.slot].exp += cur.exp;
			out += "|" + levelUps;
			// New Evs
